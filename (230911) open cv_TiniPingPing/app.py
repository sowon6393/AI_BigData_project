...
import joblib
import cv2
randomforest = joblib.load('OpenCV/cgi-bin/RandomForestClassifier(15).pkl')
characterDict = {
    'agaPing':'아자핑',
 'banglePing':'방글핑',
 'baroPing':'바로핑',
...
def upload_file():
    ...
    if file:
        filename = os.path.join("OpenCV/cgi-bin/static/upload/uploaded_image.png")
        file.save(filename)

        # 처리할 이미지
        imgData = cv2.imread("OpenCV/cgi-bin/static/upload/uploaded_image.png")
